...
function visiblePeople(record){return(record.people||[]).filter(p=>(!employmentFilter.value||p["Employment Category"]===employmentFilter.value)&&(!sgFilter.value||p["SG"]===sgFilter.value)&&(!sectionFilter.value||p["Section Name"]===sectionFilter.value));}
function markerSvg(count,selected=false){const maxCount=Math.max(...records.map(r=>visiblePeople(r).length),1),n=Math.sqrt(Math.max(count,1))/Math.sqrt(maxCount),radius=8+n*13,h=Math.ceil(radius*2+12),fill=selected?"#20419A":"#00A19C";const svg='<svg xmlns="http://www.w3.org/2000/svg" width="'+h+'" height="'+h+'" viewBox="0 0 '+h+' '+h+'"><defs><filter id="s" x="-50%" y="-50%" width="200%" height="200%"><feDropShadow dx="0" dy="2" stdDeviation="2.2" flood-color="#073f43" flood-opacity=".34"/></filter></defs><path d="M '+(h/2)+' '+(h-1)+' C '+(h/2-2)+' '+(h*.78)+' 2 '+(h*.57)+' 2 '+(h*.35)+' A '+(h/2-2)+' '+(h/2-2)+' 0 1 1 '+(h-2)+' '+(h*.35)+' C '+(h-2)+' '+(h*.57)+' '+(h/2+2)+' '+(h*.78)+' '+(h/2)+' '+(h-1)+' Z" fill="'+fill+'" fill-opacity=".92" stroke="#fff" stroke-width="2" filter="url(#s)"/><circle cx="'+(h/2)+'" cy="'+(h*.35)+'" r="'+Math.max(3,radius*.22)+'" fill="#fff" fill-opacity=".9"/></svg>';return"data:image/svg+xml;charset=utf-8,"+encodeURIComponent(svg);}
function renderLayer(){const entities=records.map(record=>{const people=visiblePeople(record);if(!people.length)return null;const selected=selectedRecord&&selectedRecord.nationality===record.nationality;return new Entity({name:record.nationality,lonlat:[record.longitude,record.latitude],billboard:{src:markerSvg(people.length,selected),size:[44,44],offset:[0,20]},label:{text:record.nationality+" · "+people.length,size:selected?16:12,offset:[0,30,0],color:selected?"rgba(32,65,154,.98)":"rgba(24,50,56,.94)",outlineColor:"rgba(255,255,255,.96)",outline:2},properties:{nationality:record.nationality,personnel:people.length,representation:record.representation,latitude:record.latitude,longitude:record.longitude}})}).filter(Boolean);nationalityLayer.setEntities(entities);const total=entities.reduce((sum,e)=>sum+Number(e.properties?.personnel||0),0);stats.textContent=total+" visible personnel · "+entities.length+" nationalities · Click a pin for details";}
function escapeHtml(value){return String(value).replace(/[&<>"']/g,ch=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[ch]));}
function showDetails(record){selectedRecord=record;const people=visiblePeople(record);document.getElementById("detailNationality").textContent="🌐 "+record.nationality;document.getElementById("detailRepresentation").textContent="Representation: "+(record.representation||"—");document.getElementById("detailCount").textContent=people.length;document.getElementById("detailSections").textContent=new Set(people.map(p=>p["Section Name"]).filter(Boolean)).size;document.getElementById("people").innerHTML=people.length?people.slice(0,60).map(p=>'<div class="person"><strong>'+escapeHtml(p["Name"]||"Unnamed")+'</strong><br>'+escapeHtml(p["Staff Position"]||"Position not specified")+" · "+escapeHtml(p["SG"]||"SG —")+'<br><span class="muted">'+escapeHtml(p["Department"]||"Department —")+" · "+escapeHtml(p["Section Name"]||"Section —")+"</span></div>").join(""):'<div class="muted">No personnel match the current filters.</div>';details.classList.add("visible");renderLayer();}
function flyToRecord(record){if(!globe||!globe.planet?.camera)return;globe.planet.camera.setLonLat(new LonLat(record.longitude,record.latitude,3500000),new LonLat(record.longitude,record.latitude,0));}
...
try{nationalityLayer=new Vector("Nationalities",{entities:[],clampToGround:true,async:false,pickingEnabled:true,scaleByDistance:[10000,5000000,1]});globe=new Globe({target:"globus",name:"RE Nationality Intelligence Globe",terrain:new GlobusRgbTerrain(),layers:[osm,nationalityLayer],atmosphereEnabled:true,resourcesSrc:"__RESOURCES__",fontsSrc:"__FONTS__",msaa:4,idleMode:true,navigation:{mode:"north",inertia:.18,zoomSpeed:1.15}});if(control.KeyboardNavigation)globe.renderer.addControl(new control.KeyboardNavigation());globe.renderer.events.on("lclick",e=>{const picked=e.pickingObject;if(!picked||!picked.properties||!picked.properties.nationality)return;const record=records.find(r=>r.nationality===picked.properties.nationality);if(record){showDetails(record);flyToRecord(record)}});renderLayer();status.classList.add("hidden");window.setTimeout(()=>window.dispatchEvent(new Event("resize")),250)}catch(error){console.error("OpenGlobus Phase 2 initialization failed:",error);status.textContent="3D Earth could not be initialized. Check WebGL/network access.";}
</script></body></html>'''
