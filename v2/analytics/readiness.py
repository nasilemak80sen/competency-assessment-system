...
def build_readiness_detail_dataframe(personnel_dataframe, ruler_map, target_mode, selected_target_sg=None):
    ...
    for dataframe_index, person_row in personnel_dataframe.iterrows():
        ...
        staff_id = _rg_clean_value(person_row.get("Staff ID")); personnel_id = person_row.get("id")
        department = _rg_clean_value(person_row.get("Department"), "Not Specified"); position = _rg_clean_value(person_row.get("Staff Position"), "Not Specified")
        employment = _rg_clean_value(person_row.get("Employment Category"), "Not Specified"); current_sg = _rg_clean_value(person_row.get("SG"), "").upper()
        ...
        requirements = ruler_map.get(ruler, {}); target_sg = _rg_determine_target_sg(current_sg, requirements, target_mode, selected_target_sg)
        if target_sg is None: continue
        for code in SCORE_COLS:
            if code not in requirements.get(target_sg, {}): continue
            target = pd.to_numeric(requirements[target_sg].get(code), errors="coerce")
            if pd.isna(target): continue
            actual = pd.to_numeric(person_row.get(code), errors="coerce"); assessed = pd.notna(actual)
            gap = float(actual)-float(target) if assessed else np.nan; capped = min(float(actual), float(target)) if assessed and target > 0 else 0.0; burden = abs(min(gap, 0)) if pd.notna(gap) else 0.0
...
def build_personnel_readiness_summary(detail_dataframe):
    ...
    for values, person in detail_dataframe.groupby(groups, dropna=False):
        group_record = dict(zip(groups, values)); total = len(person); assessed = int(person["Is Assessed"].sum()); met = int(person["Is Met"].sum()); major = int(person["Is Major Gap"].sum()); minor = int(person["Is Minor Gap"].sum())
        coverage = assessed/total*100 if total else 0.0; strict = met/total*100 if total else 0.0; target_total = person["Target Score"].fillna(0).clip(lower=0).sum(); weighted = person["Capped Actual"].sum()/target_total*100 if target_total > 0 else 0.0
        burden = person["Gap Burden"].sum(); status = classify_readiness_status(weighted,strict,coverage,major); action = recommend_readiness_action(status,coverage,major,minor); cat = {}
...
# Explicit function definitions are intentional: the migration audit tracks
# top-level legacy function names, not assignment aliases.
...
def _classify_readiness_status(*args, **kwargs): return classify_readiness_status(*args, **kwargs)
def _recommend_readiness_action(*args, **kwargs): return recommend_readiness_action(*args, **kwargs)
def _find_top_personnel_gap(*args, **kwargs): return find_top_personnel_gap(*args, **kwargs)
def _apply_readiness_personnel_filters(*args, **kwargs): return apply_readiness_personnel_filters(*args, **kwargs)

def _build_filter_options(dataframe, column_name):
    if dataframe is None or dataframe.empty or column_name not in dataframe.columns: return []
    return sorted(dataframe[column_name].dropna().astype(str).str.strip().loc[lambda s: s.ne("")].unique().tolist())


def _reset_readiness_filters(session_state):
    for key in ["rg_search","rg_department","rg_position","rg_sg","rg_ruler","rg_employment","rg_target_mode","rg_selected_target_sg","rg_target_sg_disabled","rg_coverage","rg_status","rg_ranking_metric","rg_heatmap_scope","rg_priority_person"]:
        session_state.pop(key, None)


def normalize_ruler_type(value): return _rg_normalize_ruler(value)
def sort_grades(grades): return _rg_sort_salary_grades(grades)

def build_target_gap_dataframe(person_row, target_requirements, current_sg, target_sg, target_position=None, ruler_type=None, competency_names=None):
    competency_names = competency_names or COMPETENCY_FULLNAMES
    records=[]
    for code in SCORE_COLS:
        target_value=target_requirements.get(code)
        if target_value is None or pd.isna(target_value): continue
        target=float(target_value)
        if target <= 0: continue
        actual=pd.to_numeric(person_row.get(code), errors="coerce")
        gap=(actual-target) if pd.notna(actual) else np.nan
        status="Not Assessed" if pd.isna(actual) else ("Met" if gap >= 0 else ("Minor Gap" if gap >= -1 else "Major Gap"))
        records.append({"Competency":code,"Competency Name":competency_names.get(code,code),"Current Grade":current_sg,"Target Grade":target_sg,"Target Position":target_position,"Ruler Type":ruler_type,"Actual":actual,"Target":target,"Gap":gap,"Status":status})
    return pd.DataFrame(records)


def calculate_readiness_metrics(gap_df):
    if gap_df is None or gap_df.empty: return {"total":0,"assessed":0,"not_assessed":0,"met":0,"minor":0,"major":0,"strict_readiness":0.0,"weighted_readiness":0.0}
    assessed=gap_df[gap_df["Status"]!="Not Assessed"]; total=len(gap_df); n=len(assessed); met=int((gap_df["Status"]=="Met").sum()); minor=int((gap_df["Status"]=="Minor Gap").sum()); major=int((gap_df["Status"]=="Major Gap").sum()); target=assessed["Target"].astype(float).sum() if n else 0
    achieved=np.minimum(assessed["Actual"].astype(float),assessed["Target"].astype(float)).sum() if n else 0
    return {"total":total,"assessed":n,"not_assessed":total-n,"met":met,"minor":minor,"major":major,"strict_readiness":met/n*100 if n else 0.0,"weighted_readiness":achieved/target*100 if target else 0.0}
