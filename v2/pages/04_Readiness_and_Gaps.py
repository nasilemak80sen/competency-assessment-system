...
with target_sg_col:
    if target_mode == "Selected target grade":
        ...
        st.text_input("Selected Target SG", value="Automatically determined", disabled=True, key="rg_target_sg_disabled")
with reset_col:
...
if selected_rulers:
    ruler_series = filtered_personnel_df.apply(lambda row: str(_rg_get_person_ruler(row, ruler_map)), axis=1)
...
with st.spinner("Calculating readiness and competency gaps..."):
    detail_df = build_readiness_detail_dataframe(filtered_personnel_df, ruler_map, target_mode, selected_target_sg)
...
with coverage_col:
    coverage_range = st.slider("Assessment Coverage (%)", 0, 100, (0, 100), 5, key="rg_coverage")
with status_col:
    selected_statuses = st.multiselect("Readiness Status", READINESS_STATUS_ORDER, key="rg_status")
filtered_summary_df = summary_df[summary_df["Assessment Coverage %"].between(coverage_range[0], coverage_range[1], inclusive="both")].copy()
if selected_statuses:
    filtered_summary_df = filtered_summary_df[filtered_summary_df["Readiness Status"].isin(selected_statuses)]
...
kpis[1].metric("Fully Assessed", fully_assessed, f"{fully_assessed / total_personnel * 100:.0f}%" if total_personnel else None)
...
with overview_tab:
    ...
    with left:
        st.plotly_chart(_create_readiness_status_chart(filtered_summary_df), width="stretch", config={"displaylogo": False, "responsive": True})
    with right:
        st.plotly_chart(_create_department_readiness_chart(filtered_summary_df), width="stretch", config={"displaylogo": False, "responsive": True})
    st.subheader("Personnel Ready for Assessment")
    ready_df = filtered_summary_df[filtered_summary_df["Readiness Status"] == "Ready"].sort_values(["Weighted Readiness %", "Strict Readiness %"], ascending=[False, False])
    ready_columns = [c for c in [
    ...
    if ready_df.empty:
        ...
        st.dataframe(ready_df[ready_columns], width="stretch", hide_index=True, column_config={
    ...
    with st.expander("Full Personnel Readiness Table"):
        overview_columns = [c for c in [
            "Name", "Staff ID", "Department", "Staff Position", "Employment Category", "Current SG", "Career Ruler", "Target SG",
            "Required Competencies", "Assessed Competencies", "Base Readiness %", "Key Readiness %", "Pacing Readiness %", "Emerging Readiness %",
            "Assessment Coverage %", "Weighted Readiness %", "Strict Readiness %", "Met Competencies", "Minor Gaps", "Major Gaps", "Gap Burden", "Readiness Status", "Recommended Action"
        ] if c in filtered_summary_df.columns]
        st.dataframe(filtered_summary_df[overview_columns].sort_values("Weighted Readiness %", ascending=False), width="stretch", hide_index=True)

with distribution_tab:
    st.subheader("Readiness Distribution and Assessment Coverage")
    st.plotly_chart(_create_readiness_box_plot(filtered_summary_df), width="stretch", config={"displaylogo": False, "responsive": True})
    left, right = st.columns(2)
    with left:
        st.plotly_chart(_create_category_readiness_heatmap(filtered_summary_df), width="stretch", config={"displaylogo": False, "responsive": True})
    with right:
        st.plotly_chart(_create_readiness_coverage_scatter(filtered_summary_df), width="stretch", config={"displaylogo": False, "responsive": True})
    with st.expander("How to interpret the coverage quadrant"):
        st.markdown("**Upper right:** High measured readiness and sufficient assessment coverage.\n\n**Lower right:** Sufficiently assessed, but competency development is required.\n\n**Upper left:** Strong measured results, but insufficient assessment evidence.\n\n**Lower left:** Both under-assessed and below readiness expectations.")

with gap_deep_dive_tab:
    st.subheader("Competency Gap Deep Dive")
    st.plotly_chart(_create_category_gap_distribution(filtered_detail_df), width="stretch", config={"displaylogo": False, "responsive": True})
    ...
    if risk_df.empty:
        ...
        left, right = st.columns(2)
        with left:
            st.plotly_chart(_create_competency_risk_matrix(risk_df), width="stretch", config={"displaylogo": False, "responsive": True})
        with right:
            ranking_metric = st.selectbox("Rank competency gaps by", ["Affected personnel", "Average gap severity", "Total gap burden", "Major-gap count"], key="rg_ranking_metric")
            st.plotly_chart(_create_top_competency_gap_chart(risk_df, ranking_metric), width="stretch", config={"displaylogo": False, "responsive": True})
        ...
        heatmap = _create_department_competency_heatmap(filtered_detail_df, {"Top 10": 10, "Top 15": 15, "All": "All"}[heatmap_scope])
        if heatmap is None:
        ...
        with st.expander("Competency Risk Detail"):
            st.dataframe(risk_df.sort_values(["Gap Burden", "Affected Personnel"], ascending=[False, False]), width="stretch", hide_index=True, column_config={
...
with prioritization_tab:
    ...
    priority_columns = [c for c in [
        "Name", "Staff ID", "Department", "Staff Position", "Current SG", "Career Ruler", "Target SG",
        "Assessment Coverage %", "Weighted Readiness %", "Strict Readiness %", "Major Gaps", "Minor Gaps", "Gap Burden", "Top Gap", "Years in Grade", "Readiness Status", "Recommended Action"
    ] if c in priority_df.columns]
    st.dataframe(priority_df[priority_columns], width="stretch", hide_index=True, column_config={
        "Assessment Coverage %": st.column_config.ProgressColumn("Coverage", min_value=0, max_value=100, format="%.0f%%"),
        "Weighted Readiness %": st.column_config.ProgressColumn("Weighted Readiness", min_value=0, max_value=100, format="%.0f%%"),
        "Strict Readiness %": st.column_config.ProgressColumn("Strict Readiness", min_value=0, max_value=100, format="%.0f%%"),
        "Gap Burden": st.column_config.NumberColumn("Gap Burden", format="%.0f"),
    ...
    if not person_options:
        ...
        if selected_summary_rows.empty:
            ...
            person_detail = filtered_detail_df[filtered_detail_df["DataFrame Index"] == selected_summary["DataFrame Index"]].copy()
            ...
            person_detail = person_detail.sort_values(["_Severity Order", "Gap", "Competency Code"], ascending=[True, True, True], na_position="last").drop(columns=["_Severity Order"])
            ...
            detail_columns = [c for c in ["Competency Code", "Competency Name", "Category", "Current SG", "Target SG", "Actual Score", "Target Score", "Gap", "Gap Severity"] if c in person_detail.columns]
            st.dataframe(person_detail[detail_columns], width="stretch", hide_index=True)

st.markdown("### ⚠️ Competency Gap Detail")
gap_detail = filtered_detail_df[filtered_detail_df["Gap"].notna() & (filtered_detail_df["Gap"] < 0)].copy()
if gap_detail.empty:
    st.success("No assessed competency gaps were found for the selected population.")
else:
    gap_columns = [c for c in ["Name", "Staff ID", "Department", "Current SG", "Target SG", "Category", "Competency Code", "Competency Name", "Actual Score", "Target Score", "Gap", "Gap Severity"] if c in gap_detail.columns]
