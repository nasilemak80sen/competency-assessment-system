...
if selected_people:
    ...
    if comparison_competencies:
        ...
        for idx, person_name in enumerate(selected_people):
            ...
            with comparison_cols[idx]: st.plotly_chart(figure, width="stretch", config={"displaylogo": False, "responsive": True})
    else: st.info("No competency score columns are available for comparison.")
else:
    st.info("Choose 1 to 3 personnel to view their competency radar profiles.")

st.markdown("---")
st.subheader("📈 Data Element Analysis")
if "cb_filters" not in st.session_state: st.session_state.cb_filters = {}

with st.expander("🔽 Step 1: Apply Filters (Optional)", expanded=False):
    filter_cols = st.columns(3); filters = {}
    for idx, column in enumerate(["Department", "Staff Position", "SG"]):
        if column in df.columns:
            with filter_cols[idx]:
                values = st.multiselect(column if column != "SG" else "Salary Grade (SG)", sorted(df[column].dropna().astype(str).unique()), key=f"chart_{column.lower().replace(' ','_')}_filter")
                if values: filters[column] = values
    working_df = df.copy()
    for column, values in filters.items(): working_df = working_df[working_df[column].astype(str).isin(values)]
    if filters and not working_df.empty:
        st.session_state.cb_filters = filters; st.info(f"Filters applied: Showing {len(working_df):,} of {len(df):,} records")
    elif filters:
        st.warning("Filters resulted in no records. Showing the original dataset instead."); st.session_state.cb_filters = {}; working_df = df.copy()
    else:
        st.session_state.cb_filters = {}; st.info(f"No filters applied: Using all {len(df):,} records")

if st.session_state.cb_filters:
    working_df = df.copy()
    for column, values in st.session_state.cb_filters.items(): working_df = working_df[working_df[column].astype(str).isin(values)]
if working_df.empty: st.warning("No data is available for chart construction."); st.stop()
...
with info_cols[0]: st.caption(f"**X:** {x_info.data_type.value} · {x_info.unique_count:,} unique · {x_info.null_count:,} missing")
with info_cols[1]:
    if y_info: st.caption(f"**Y:** {y_info.data_type.value} · {y_info.unique_count:,} unique · {y_info.null_count:,} missing")
...
if not compatible_names: st.warning("No compatible chart types were found for the selected elements."); st.stop()
...
if chart_type == "Bubble Chart":
    ...
    if size_options: size_col = st.selectbox("Bubble size", size_options, key="cb_size_element")
...
if st.button("📊 Generate Chart", type="primary", width="stretch"):
    try:
        ...
        with st.expander("Error details"): st.exception(exc)
else:
