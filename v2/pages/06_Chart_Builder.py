...
from analytics.charts import ChartBuilder, ChartCompatibility

st.title("📊 Dynamic Chart Builder")
st.markdown(
    "Create custom charts by selecting data elements from the personnel dataset. "
    "The builder detects data types, recommends compatible chart types, and supports filters."
)
st.markdown("---")
...
selected_people = st.multiselect(
    "Select up to 3 personnel", personnel_names, max_selections=3,
...
if selected_people:
    ...
    if comparison_competencies:
        ...
        for idx, person_name in enumerate(selected_people):
            ...
            theta_closed = comparison_competencies + [comparison_competencies[0]] if comparison_competencies else []
            figure = go.Figure()
            figure.add_trace(go.Scatterpolar(
                r=values_closed, theta=theta_closed, fill="toself", name=person_name,
                line=dict(color="#00a19c", width=2), fillcolor="rgba(0, 161, 156, 0.35)",
            ))
            figure.update_layout(
                title=person_name, height=320,
                polar=dict(radialaxis=dict(visible=True, range=[0, 5], dtick=1), bgcolor="rgba(240,240,240,0.15)"),
...
# -----------------------------------------------------------------------------
# Dynamic data-element chart builder
# -----------------------------------------------------------------------------
...
with st.expander("🔽 Step 1: Apply Filters (Optional)", expanded=False):
    ...
    if "Department" in df.columns:
        with filter_cols[0]:
            values = st.multiselect("Department", sorted(df["Department"].dropna().astype(str).unique()), key="chart_dept_filter")
            if values: filters["Department"] = values
    if "Staff Position" in df.columns:
        with filter_cols[1]:
            values = st.multiselect("Staff Position", sorted(df["Staff Position"].dropna().astype(str).unique()), key="chart_pos_filter")
            if values: filters["Staff Position"] = values
    if "SG" in df.columns:
        with filter_cols[2]:
            values = st.multiselect("Salary Grade (SG)", sorted(df["SG"].dropna().astype(str).unique()), key="chart_sg_filter")
            if values: filters["SG"] = values
...
# Use the same compatibility analyzer as the legacy builder rather than
# maintaining a second set of type rules in the page.
element_names = [c for c in working_df.columns if c != "id"]
x_col = st.selectbox("X-axis data element", element_names, key="cb_x_element")
y_options = ["(None)"] + [c for c in element_names if c != x_col]
y_selection = st.selectbox("Y-axis data element", y_options, key="cb_y_element")
